...
# Function to plot all the graphs together
def plot_all_graphs(df):
    ...
    df['taarek'] = df['Date']
    daily_timeline = df.groupby('taarek').count()['Message'].reset_index()
    axes[0, 0].plot(daily_timeline['taarek'], daily_timeline['Message'])
    axes[0, 0].set_title('Daily Timeline')
    axes[0, 0].set_ylabel("Messages Sent")

    # Weekly activity
    x = df['day'].value_counts()
    axes[0, 1].bar(x.index, x.values)
    axes[0, 1].set_title('Weekly Activity')
    axes[0, 1].set_xlabel("Days")
    axes[0, 1].set_ylabel("Message Sent")
    axes[0, 1].tick_params(axis='x', rotation=45)

    # Monthly activity
    x = df['month_name'].value_counts()
    axes[1, 0].bar(x.index, x.values)
    axes[1, 0].set_title('Monthly Activity')
    axes[1, 0].set_xlabel("Months")
    axes[1, 0].set_ylabel("Message Sent")
    axes[1, 0].tick_params(axis='x', rotation=45)

    # Emoji analysis
    emojiDF = getEmoji(df)
    axes[1, 1].pie(emojiDF[1].head(), labels=emojiDF[0].head(), autopct="%0.2f", shadow=True)
    axes[1, 1].set_title('Emoji Analysis')

    # Most common words
    commonWord = MostCommonWords(df)
    axes[2, 0].bar(commonWord[0], commonWord[1])
    axes[2, 0].set_title('Most Frequent Words Used In Chat')
    axes[2, 0].set_xlabel("Words")
    axes[2, 0].set_ylabel("Frequency")
    axes[2, 0].tick_params(axis='x', rotation=45)

    # Activity heatmap
    heatmap_data = activity_heatmap(df)
    sns.heatmap(heatmap_data, ax=axes[2, 1], cmap="YlGnBu")
    axes[2, 1].set_title('Activity Heatmap')
...
if __name__ == "__main__":
    ...
    with open(file, 'r', encoding='utf-8') as f:
        data = f.read()
    # rest of your code'  # Replace with your file path
